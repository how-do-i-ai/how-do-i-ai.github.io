...
- **CI** runs on every push and PR (`.github/workflows/ci.yml`): security audit (`npm audit --omit=dev`), lint, typecheck, unit tests (`npm run test`), build, Playwright suites (QA-07 touch-target audit + QA-08 axe-core WCAG 2.2 AA + QA-09 visual regression), Phase 1 QA-10 audit steps (QA-10.1 route clustering + QA-10.2 critical widths + QA-10.3 invariant specs, each invoking the corresponding `npm run test:audit:*` script), and Lighthouse mobile gates (QA-06, `npx lhci autorun`). All must pass before merge.
...
| **QA-07 Touch-target audit** | Every `<a> / <button> / <details summary> / <input>` on `/`, `/blog/`, `/blog/the-30-second-ai-explainer/` ≥ 44×44px at 320/375/414/768                                                      | `npm run test:touch-targets` |
