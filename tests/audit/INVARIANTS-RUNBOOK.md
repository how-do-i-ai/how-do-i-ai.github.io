...
3. **A new page type enters the canonical surface set.** Today that set is home (`/`), blog index (`/blog/`), and a representative post (`/blog/the-30-second-ai-explainer/`) — see [`tests/visual/README.md`](../visual/README.md) § "What this guards". When QA-10.1 route clustering ([`tests/audit/routes.spec.ts`](./routes.spec.ts) + [`ROUTE-CLUSTERS-RUNBOOK.md`](./ROUTE-CLUSTERS-RUNBOOK.md) once landed) registers a new archetype, evaluate whether the new page type carries design decisions that warrant invariants of its own.
...
- `invariant_footer_tagline_about_aligned_PDR_007_149` (Invariant 8 — PDR-007 discovery case, issue #149)
- `invariant_footer_social_row_no_widow_PDR_007_151` (Invariant 10 — PDR-007 discovery case, issue #151)
...
- [Issue #152](https://github.com/how-do-i-ai/how-do-i-ai.github.io/issues/152) — Invariant 11 (`.hero-descriptor` last-line ≥6 characters) — non-accent short-widow class discovered via `/review-ui` pass on QA-09 baselines (§ When to add trigger 2). Complements Invariant 3 (accent-orphan): same `.hero-tagline`-family elements, orthogonal predicate (count vs membership). Paired reverse test in [`invariants.reverse.spec.ts`](./invariants.reverse.spec.ts) injects `text-wrap: auto` to re-produce the "do?" widow.
- [Issue #149](https://github.com/how-do-i-ai/how-do-i-ai.github.io/issues/149) — Invariant 8 (footer tagline + About link share a left coordinate in stacked layout) — column-alignment class discovered via `/review-ui` pass on QA-09 baselines (§ When to add trigger 2). Forward invariant is a FORWARD-COMPAT guard on element-rect alignment; the user-visible rendered-text drift is gated by QA-09 pixel baselines regenerated with the fix (see the Invariant 8 docblock in [`invariants.spec.ts`](./invariants.spec.ts) and [`helpers.ts`](./helpers.ts) for the scope note). Paired reverse test in [`invariants.reverse.spec.ts`](./invariants.reverse.spec.ts) injects `margin-inline-start: 24px` via DOM mutation to shift the link rect right of the tagline rect — the OS-robust mutation pattern matches Invariants 9 + 11 rather than a CSS `addStyleTag` override.
- [Issue #151](https://github.com/how-do-i-ai/how-do-i-ai.github.io/issues/151) — Invariant 10 (footer social row wraps with no single-icon widow on the final row) — odd-count flex-wrap class discovered via `/review-ui` pass on QA-09 baselines (§ When to add trigger 2). The fix uses a zero-height `.wrap-spacer` `<li>` with `flex-basis: 100%`, gated to a targeted media query `(max-width: 374px), (min-width: 768px) and (max-width: 845px)` that covers both the 320 natural-widow case (2+3+3+1 → 2+3+2+2) and the mid-tablet 768-845 widow band (8+1 → 7+2). The predicate groups `.channel-link` elements by top coordinate with a 1px tolerance (matches Invariants 3 + 11 + 6). Paired reverse test in [`invariants.reverse.spec.ts`](./invariants.reverse.spec.ts) removes the `.wrap-spacer` element via DOM mutation at 320, re-exposing the pre-fix wrap distribution — the OS-robust mutation pattern matches Invariants 8 + 9 + 11.
